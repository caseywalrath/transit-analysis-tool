...
The Layers tab on the right feature panel (`js/core/layers-panel.js`, sibling of the Features tab) lists drawn feature groups plus Analysis and Reference/Imported bands, with per-layer visibility toggles, opacity, constrained drag-reorder, a basemap selector, and — below a "Style defaults" heading — per-type style drawers (color, opacity, weight) and per-feature override drawers. Color is a live default, not a one-time stamp: it resolves fresh on every render (`App.resolveFeatureColor`, `docs/feature-color-system-plan.md`), so changing a type's color swatch retroactively recolors every feature of that type that doesn't carry its own override. Each type can also be set to **Automatic** (the swatch clears back to it via a small clear button) — for Lines/Routes that means every feature keeps its own distinct color from a shared palette instead of one flat color; Points/Polygons have no rainbow mode, so Automatic there is just the built-in default. A per-feature color override gets the same clear button once set. Modeled on Felt's layers panel.
