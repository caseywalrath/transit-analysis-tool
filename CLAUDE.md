...
**Covered engines:** Ridership Forecasting, TPI scoring, Route Costing, Trip Builder, Corridor Scoring, Transit Coverage, Module Buffers, Travelshed, Choropleth classification. **Deferred intentionally:** Title VI (see the note in `features.md`). The harness pins only *pure* math — no map, DOM, Census/LODES API, or turf geometry; those paths are out of scope by design, so not every code change needs a test run, only ones touching calculation logic.
...
js/
  ...
  core/
    ...
    features.js             Right-side feature panel: lists features, editable names, per-item color swatches, gear icon (⚙) per row to open the floating attributes popup, right-click context menu with Attributes option. Row click selects/highlights feature on map only. Delete (trash icon) stays in the row with an inline confirm strip. The main Features list is sortable (Name/Type/Date added/Group, ascending/descending, show/hide groups) via a sort icon button in the panel header (`#fp-sort-btn`) or right-click on the header — display-only, never reorders `App.points`/`lines`/`routes`/`polygons` or map draw order; the Labels and Text section is unaffected and always sorts by name. Exports refreshFeaturePanel, openColorPicker, updateFeatureColor.
    ...
    choropleth.js           Shared choropleth rendering engine (`App.choropleth`), first consumer Feature Area Analysis; TPI/Ridership Forecasting/Corridor Scoring migrate onto it in Phase 3 (see `docs/feature-area-choropleth-plan.md`). Pure, golden-testable classification math — `computeClassBreaks` (quantile/equal, `nClasses-1` inner breaks, dedup on heavily tied data, degenerate all-equal/single-value/empty cases), `buildStepColorExpr` (MapLibre `step` paint expression wrapped so non-numeric/missing renders a no-data color instead of the first class), `formatBreakLabels` — plus a map-facing instance API keyed by an `id` string so multiple modules can each own one live choropleth without colliding: `render(opts)` (creates/updates source `"<id>-choropleth"` + layers `"<id>-choropleth-fill"`/`"-line"`, same create-once/setData-after + hover-popup pattern as TPI's original inline version; returns `{breaks, colors, nEffective, min, max}` for the caller's legend), `remove(id)` (idempotent), `setVisible(id, bool)`, and `fillLegend(containerEl, opts)` for the generic legend fragment below. `RAMPS` holds the curated ColorBrewer presets (`blues` default, `heat`, `greens`, `rdbu`).
  ...
  projects/
    buffer-summary.js       Feature Area Analysis module: MANDATORY_VARS, expandGroups, runSummary, buildVarChecklistHTML, and a points/lines/routes/polygons checklist. It can use a 0.5-mi default analysis distance or selected displayed buffers; polygons pass through unbuffered. Registered as popup-based module. **Choropleth (Phase 1 of `docs/feature-area-choropleth-plan.md`):** `runSummary()` retains every fetched variable's per-geography values (`_lastGeoData.perGeo`, keyed by var code, `{level: "geo"|"tract", values: Map<GEOID, number>}` — tract-fallback variables keep their tract-level map) alongside the geometry, overlap fractions, and denominator specs needed to redisplay them — no additional fetches. A "Map by geography" dropdown (`#basMapVar`, populated by `populateBasMapVarDropdown()`) lists every displayed variable with per-geo data (LODES excluded until Phase 2; tract-fallback entries labeled "(tract level)"); picking one renders a 5-class quantile Blues choropleth via `App.choropleth.render({id: "bas", ...})` colored by **whole-geography values** (clipped geometry when apportioned, uncut values — the settled design decision, so a large geography barely clipped by the buffer never reads as "low"), with a hover popup (`basHoverHTML`) showing the hovered variable plus its percent/apportioned share and every other selected variable's value at that geography, and a floating legend (`projects/choropleth-legend.html` widget id `"bas-legend"`) filled via `App.choropleth.fillLegend`. A `#basHideChoropleth` checkbox toggles visibility without re-rendering; selecting "None" removes the choropleth and restores the plain gray `census-geos` overlay. `#basStatus` (`App.renderModuleState`) shows Done/error, and a **stale-but-visible** banner with a working Re-run button when features change while results are on screen (the module deliberately does not auto-clear on feature deletion, unlike Corridor Scoring/Transit Coverage). A `clear` lifecycle hook (wired to the toolbar Clear/Reset Session buttons) removes the choropleth, hides the legend, and empties the census overlay so neither button leaves orphaned map layers. The selected map variable persists as an additive `mapVar` cache field (geometry/results are not persisted, so a restored session's dropdown re-selects and the choropleth re-renders automatically on the next Calculate Summary, matching the Transit Coverage/Transit Travelshed settings-only precedent).
...
projects/
  buffer-summary-popup.html   Feature Area Analysis popup body: vertical setup/results layout on the shared `.rf-section-row` / `.rf-settings-col` / `.rf-results-col` schema. Settings has an all-feature checklist, 0.5-mi default analysis distance, Use Display Buffers toggle, variable checklist, geography, ACS year, apportion toggle, and Calculate Summary; results has the `#basStatus` status pill, progress line, the choropleth `#basMapRow` (`#basMapVar` dropdown + `#basHideChoropleth` toggle, hidden until a run succeeds), `#basEmptyState`, table, and notes.
  choropleth-legend.html        Generic choropleth legend fragment shared by `App.choropleth`'s map-facing API (`docs/feature-area-choropleth-plan.md`): a `.tpi-legend-panel`-style shell with a `.cl-legend-title` line, 5 pre-built `.cl-legend-row` rows (each a `.cl-legend-swatch` + `.cl-legend-label`, unused rows hidden by `fillLegend`), and a `.cl-legend-note` line. Filled after the floating widget mounts via `App.choropleth.fillLegend(containerEl, {title, labels, colors, note})` — the same fill-after-mount pattern as `transit-travelshed-legend.html`. First consumer is Feature Area Analysis (widget id `"bas-legend"`).
...
### census.js
`renderCensusOverlay(geos)`, `clearCensusOverlay()` (empties the `census-geos` source's data via `setData` rather than removing the source/layers — same create-once/setData-after pattern the choropleth engine uses, so the layers are ready to reuse on the next overlay), `fetchAllTigerwebFeatures(layerUrl, params)`, `fetchTigerwebGeos(geoLevel, unionFeat)`, `parseGEOID(geoLevel, geoid)`, `fetchACSValues(geoLevel, year, varCode, geoids)`, `fetchACSCountyValues(year, varCode, counties)`, `aggregateWithinUnion(unionFeat, geos, valueMap, aggMode, options)`, `computeGeoOverlapFractions(unionFeat, geos, apportionByArea)` (→ `Map<GEOID, frac>`; the same per-geo overlap-fraction logic `aggregateWithinUnion` computes inline, exposed so a caller can compute it once per run and reuse it — `aggregateWithinUnion`'s optional `options.fractions` accepts a precomputed map, falling back to its original inline computation when absent), `computeAcsValueOnly(varCode, year, geoLevel)`
...
### choropleth.js
Shared choropleth rendering engine (`App.choropleth`), first consumer Feature Area Analysis. See `docs/feature-area-choropleth-plan.md` for the full design (curated ramp presets, apportioned-value display rule, Phase 3 migration of TPI/RF/Corridor Scoring onto this engine).

`App.choropleth.RAMPS` — curated ColorBrewer presets, each `{ label, colors5 }`: `blues` (default), `heat` (Yl-Or-Rd), `greens`, `rdbu` (diverging Rd-Bu).

`App.choropleth.computeClassBreaks(values, method, nClasses)` — pure, golden-tested. `values` is an array of finite numbers (caller pre-filters null/NaN); `method` is `"quantile"` (linear-interpolated rank position, "R-7" convention) or `"equal"`; `nClasses` defaults to 5. Returns `{ breaks, nEffective }` — `breaks` is `nClasses - 1` **inner** break values (class *i* covers `breaks[i-1] < v <= breaks[i]`), deduplicated so heavily tied data yields `nEffective < nClasses`; all-equal values yield `{ breaks: [], nEffective: 1 }`; empty input yields `{ breaks: [], nEffective: 0 }`.

`App.choropleth.buildStepColorExpr(prop, breaks, colors, noDataColor)` — pure. Builds a MapLibre `["step", ["get", prop], ...]` paint expression from `breaks`/`colors` (`colors.length` must equal `breaks.length + 1`), wrapped in a `["case", ["==", ["typeof", ["get", prop]], "number"], stepExpr, noDataColor]` so a non-numeric/missing property renders `noDataColor` (default `"rgba(200,200,200,0.35)"`) instead of silently taking the first class.

`App.choropleth.formatBreakLabels(breaks, min, max, fmt)` — pure. Returns one human-readable range label per class, lowest class first (e.g. `"1,204 – 2,410"`); `fmt` is a `(number) => string` callback, defaulting to `String`.

`App.choropleth.render(opts)` — map-facing; `opts = { id, features, valueProp, method, classes, ramp, breaks (optional manual/fixed override), beforeLayer, fillOpacity (default 0.55), lineColor/lineWidth/lineOpacity, hoverHTML (fn(props) => html string | falsy, or null for no hover), noDataColor }`. Creates or updates source `"<id>-choropleth"` and layers `"<id>-choropleth-fill"`/`"<id>-choropleth-line"` (create-once/`setData`-after, `beforeLayer` defaults to `"buffers-fill"` when present), attaching a shared hover `maplibregl.Popup` with cursor management on first creation. Keyed by `id` so multiple modules can each own one live choropleth instance without colliding (e.g. `"bas"` for Feature Area Analysis). Returns `{ breaks, colors, nEffective, min, max }` for the caller's legend.

`App.choropleth.remove(id)` — removes both layers + source and detaches hover listeners. Idempotent.

`App.choropleth.setVisible(id, visible)` — layout-visibility toggle (e.g. a "hide choropleth" checkbox). No-op if the layers don't exist.

`App.choropleth.fillLegend(containerEl, opts)` — fills the generic legend fragment (`projects/choropleth-legend.html`) once its floating-widget body has mounted (the Transit Travelshed fill-after-mount precedent). `opts = { title, labels, colors, note }`; rows fill top-to-bottom matching `labels`/`colors` order (lowest class first), unused rows hidden.
